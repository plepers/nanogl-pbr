...
ProgramCache.prototype = {
  ...
  compile : function( material ){
    ...
    hash = inputs.getHash();
    hash +=  material._uid + material.precision;
    ...
    var chunks = inputs.getChunks();

    var vert = this.vertSrc
      .replace( PRAGMA_P, chunks.pv )
      .replace( PRAGMA_C, chunks.v  );

    var frag = this.fragSrc
      .replace( PRAGMA_P, chunks.pf )
      .replace( PRAGMA_C, chunks.f  );

    var defs = 'precision ' + material.precision + ' float;\n';
