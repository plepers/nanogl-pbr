...
CommonChunk.prototype.genCode = function( slots ){
  ...
  code =  'highp float roughness = -10.0 / log2( gloss*0.968+0.03 );\n'+
          'roughness *= roughness;\n'+
          'float specularMul = roughness * (0.125/PI) + 0.5/PI;\n';
...
ShadowsChunk.prototype.genCode = function( slots ){
  ...
  if( this.shadowCount > 0 ){
    var code = 'highp vec2 kernelOffset = uShadowKernelRotation * SHADOW_KERNEL;'
    slots.add( 'lightsf', code );

    slots.add( 'pf', shadPreCode( this.setup ) );
  }
...
ShadowsChunk.prototype.addLight = function( light ){
  ...
  this._matrices       .set( light.getShadowProjection(), i*16*4 );
  this._texelBiasVector.set( light.getTexelBiasVector(),  i*4*4 );
...
ShadowsChunk.prototype.setup = function( ){
  // TODO 
};
...
LightDatas.prototype.setup = function( prg ){
  for (var i = 0; i < this.shadowIndices.length; i++) {
    ...
    if( si > -1 ){
      prg[ 'tShadowMap'+si ]( this.lights._fbo.color );
...
SpotDatas.prototype.setup = function( prg ){
  LightDatas.prototype.setup.call( this );
