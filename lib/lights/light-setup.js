...
ShadowsChunk.prototype.addLight = function( light ){
  ...
  if( i===0 ){
    this.lightSetup.depthFormat.set( light._fbo.isDepthTexture() ? 'D_DEPTH' : 'D_RGB' );
  }
...
LightDatas.prototype.setup = function( prg ){
  for (var i = 0; i < this.shadowIndices.length; i++) {
    ...
    if( si > -1 ){
      var fbo  =this.lights[i]._fbo;
      var tex = fbo.isDepthTexture() ? fbo.getDepthTexture() : fbo.color;
